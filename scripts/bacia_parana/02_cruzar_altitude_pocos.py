...
df['LONGITUDE_BASE_DD'] = pd.to_numeric(df['LONGITUDE_BASE_DD'].astype(str).str.replace(',', '.'), errors='coerce')
df['LATITUDE_BASE_DD'] = pd.to_numeric(df['LATITUDE_BASE_DD'].astype(str).str.replace(',', '.'), errors='coerce')
...
with rasterio.open(raster_path) as src:
    ...
    x_proj, y_proj = transformer.transform(df['LONGITUDE_BASE_DD'].values, df['LATITUDE_BASE_DD'].values)
