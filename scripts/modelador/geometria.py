...
def obter_grid_interpolado(
    ...
    if unique_pts.shape[0] < 3:
        return np.full(
            (resolucao_grid, resolucao_grid), np.nanmean(z_vals), dtype=float
        )
    ...
    escalonamento = np.array([[1.0, 0.0], [0.0, 1.0 / razao_anisotropia]])
    ...
    print(
        f"[DEBUG] Grid interpolado gerado com forma: {z_grid.shape} e valores NaN: {np.isnan(z_grid).sum()}"
    )
    return z_grid
